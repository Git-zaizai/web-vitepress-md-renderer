```ts
import { componentPlugin, type ComponentPluginOptions } from '@mdit-vue/plugin-component';
...
import type { LanguageInput, ShikiTransformer, ThemeRegistrationAny } from '@shikijs/types';
...
// shiki 可以解析的语言列表 highlight.ts 93 行
let _getLoadedLanguages: () => string[];
...
export async function createMarkdownRenderer(
  ...
  md.use(componentPlugin, { ...options.component })
    ...
    .use(linkPlugin, { target: '_blank', rel: 'noreferrer', ...options.externalLinks }, base, slugify)
  ...
  md.renderer.rules.table_open = function (tokens, idx, options, env, self) {
    ...
    return tableOpen ? tableOpen(tokens, idx, options, env, self) : self.renderToken(tokens, idx, options);
  };
